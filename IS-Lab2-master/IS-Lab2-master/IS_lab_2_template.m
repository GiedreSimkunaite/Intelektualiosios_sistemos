...
y = zeros(1, 200);
x1_test = linspace(x1_min, x1_max, 200);
for i=1:200
    for j=1:6
        v = v1x(w1x(j), x1_test(i), b1x(j));
        y(i) = y(i) + y2x(v)*w2x(j);
    end
    y(i) = y(i) + b21;
end
...
% % Multi-layered perceptron testing
y = zeros(1, 200);
x2_test = linspace(x2_min, x2_max, 200);
for i=1:200
    for j=1:6
        v = Av1x(w11x(j), x1_test(i), w12x(j), x2_test(i), b1x(j));
        y(i) = y(i) + y2x(v)*w2x(j);
    end
    y(i) = y(i) + b21;
end
